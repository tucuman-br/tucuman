...
def run_cmd(cmd, timeout=TIMEOUT_SEC):
    ...
    try:
        ...
        return "NOT_INSTALLED: CPAchecker nao encontrado.", 0.0
...
def main():
    ...
    cpachecker_bin = os.path.expanduser(input("Caminho do executável CPAchecker (ex: /path/to/cpachecker): ").strip())
    cpachecker_config = os.path.expanduser(input("Caminho das propriedades (.properties): ").strip())
    ...
    logs_dir = "logs_brutos_cpachecker"
    os.makedirs(logs_dir, exist_ok=True)

    results = []
    out_csv = "resultados_brutos_cpachecker.csv"
    ...
    for root, _, files in os.walk(bench_root):
        ...
        print(f"\n[CPAchecker] Processando diretório: {subdir_name}")
        ...
        with open(txt_path, "w", encoding="utf-8") as txt_file:
            for file in c_files:
                ...
                cmd = [
                    ...
                    "--option", "parser.preprocessor=cpp -std=gnu17 -D__CPACHECKER__",
                    file_path,
                ]
