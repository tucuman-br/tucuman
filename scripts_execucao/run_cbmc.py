...
def run_cmd(cmd, timeout=TIMEOUT_SEC):
    ...
    try:
        ...
        return "NOT_INSTALLED: Comando 'cbmc' nao encontrado.", 0.0
...
def main():
    ...
    logs_dir = "logs_brutos_cbmc"
    os.makedirs(logs_dir, exist_ok=True)

    results = []
    out_csv = "resultados_brutos_cbmc.csv"

    # Percorre cada subdiretório no benchmark
    for root, _, files in os.walk(bench_root):
        ...
        print(f"\n[CBMC] Processando diretório: {subdir_name}")
        ...
        with open(txt_path, "w", encoding="utf-8") as txt_file:
            for file in c_files:
                ...
                for run_idx in range(1, repetitions + 1):
                    ...
                    # Cabeçalho do log no arquivo .txt agregador do diretório
                    header = (
                        f"{'='*80}\n"
                        f"ARQUIVO: {rel_path}\n"
    ...
    print(f"\nResultados e índice salvos em: {out_csv}")
