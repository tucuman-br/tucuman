...
def run_cmd(cmd, timeout=TIMEOUT_SEC):
    ...
    try:
        ...
        return "NOT_INSTALLED: Comando 'esbmc' nao encontrado.", 0.0
...
def main():
    ...
    logs_dir = "logs_brutos_esbmc"
    os.makedirs(logs_dir, exist_ok=True)

    results = []
    out_csv = "resultados_brutos_esbmc.csv"
    ...
    for root, _, files in os.walk(bench_root):
        ...
        print(f"\n[ESBMC] Processando diretório: {subdir_name}")
        ...
        with open(txt_path, "w", encoding="utf-8") as txt_file:
            for file in c_files:
                ...
                for run_idx in range(1, repetitions + 1):
                    cmd = ["esbmc", "-D__ESBMC__", file_path]
