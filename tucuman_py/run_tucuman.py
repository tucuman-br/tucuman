...
def run_cmd(cmd, timeout=TIMEOUT_SEC, cwd=None):
    start = time.time()
    try:
        result = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, timeout=timeout, cwd=cwd)
        output = result.stdout
    except FileNotFoundError:
        return "NOT_INSTALLED: Tucuman/Python nao encontrado.", 0.0
...
def main():
    ...
    tucuman_dir = os.path.expanduser(input("Informe o diretório do Tucuman (onde está tucuman.py): ").strip())
    ...
    verifier_path = os.path.join(tucuman_dir, "tucuman.py")
    logs_dir = "logs_brutos_tucuman"
    os.makedirs(logs_dir, exist_ok=True)

    results = []
    out_csv = "resultados_brutos_tucuman.csv"
    ...
    for root, _, files in os.walk(bench_root):
        ...
        print(f"\n[Tucuman] Processando diretório: {subdir_name}")
        ...
        with open(txt_path, "w", encoding="utf-8") as txt_file:
            for file in c_files:
                ...
                cmd = ["python3", "-u", verifier_path, os.path.abspath(file_path)]

                for run_idx in range(1, repetitions + 1):
                    raw_output, t = run_cmd(cmd, timeout=TIMEOUT_SEC, cwd=tucuman_dir)
